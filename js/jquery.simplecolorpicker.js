...
const SimpleColorPicker = function (el, opts) {
  this.$el = $(el);
  this.options = opts;
...
SimpleColorPicker.prototype = {
  ...
  init() {
    ...
    _t.config.totalHeight = Math.ceil((_t.config.colors.length) / _t.config.cols)
                * (_t.config.cellHeight + (2 * _t.config.cellBorder));
  ...
  setupInputElement() {
    ...
    var defaultColor = function () {
      if (_t.$el.val() && _t.$el.val() !== '') {
        defaultColor = _t.$el.val();
      } else {
        defaultColor = _t.config.defaultColor.hex;
      }
      return defaultColor;
    };

    const title = _t.$el.attr('name');
    const str = title.substring(0, 2);
    let _e;
    if (str == 'fr') {
      if ($('#id_sectioncolor').val() != '') {
        _e = $('#id_sectioncolor').val();
      } else {
        _e = '#000000';
      }
      _t.$el.css({
  ...
  open() {
    ...
    $(document).on(`mouseup.cp${_t.colorPickerInstance}`, 'html', () => {
      _t.close.call(_t);
    });
  },

  // Set position of picker to attach to bot-left of input.
  setPickerPosition($cp) {
    const _t = this;
    const offset = _t.$el.offset();
    const height = _t.$el.height();
    $cp.css({
  ...
  getCells() {
    const _t = this;
    const cellCount = _t.config.colors.length;
    const $cells = $('<div/>');
    let i;
    let $cell;

    for (i = 0; i < cellCount; i++) {
      $cell = $(`<li tabindex='0' title='${_t.config.colors[i].name
      }' data-color='${_t.config.colors[i].hex}'></li>`);
      $cell.css({
  ...
  getPickerMenu() {
    ...
    const $cp = $(`<ul class='${_t.config.lsClass}' id='cp-${_t.colorPickerInstance}' />`);
    let $cells;
  ...
  bindPickerEvents() {
    ...
    $cell.bind({
      ...
      mouseup(e) {
        ...
        const newColorName = $this.attr('title');
        // Update input.
